...
public class CrematoriumBlock extends MagifactureBlock {
    ...
    public static final Block INSTANCE = new CrematoriumBlock(FabricBlockSettings.create().mapColor(MapColor.GRAY).strength(3F).luminance(state -> state.get(Properties.LIT) ? 15 : 0));
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return world.isClient ? null : BlockWithEntity.validateTicker(type, CrematoriumBlockEntity.TYPE, CrematoriumBlockEntity::tickServer);
    }
