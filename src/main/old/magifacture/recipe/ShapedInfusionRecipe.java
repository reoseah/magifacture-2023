...
public class ShapedInfusionRecipe extends InfusionRecipe {
    public static final Serializer SERIALIZER = new ShapedInfusionRecipe.Serializer();
    ...
    public ShapedInfusionRecipe(Identifier id, int width, int height, DefaultedList<Ingredient> inputs, ResourceAmount<FluidVariant> fluid, ItemStack result, int duration) {
        super(id);
        this.width = width;
        this.height = height;
        this.fluid = fluid;
    ...
    public ResourceAmount<FluidVariant> getFluidCost(InfusionTableBlockEntity inventory) {
        ...
        if (this.fluid.resource().equals(inventory.getTank().variant)
            ...
            for (int i = 0; i <= 3 - this.width; ++i) {
                for (int j = 0; j <= 3 - this.height; ++j) {
                    if (this.matchesSmall(inventory, i, j, true) || this.matchesSmall(inventory, i, j, false)) {
                        return true;
                    }
                }
            }
    ...
    private boolean matchesSmall(InfusionTableBlockEntity inventory, int offsetX, int offsetY, boolean bl) {
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 3; ++j) {
                int k = i - offsetX;
                int l = j - offsetY;
    ...
    public static ItemStack getItemStack(JsonObject json) {
        String name = JsonHelper.getString(json, "item");
        Item item = Registries.ITEM.getOrEmpty(new Identifier(name)).orElseThrow(() -> new JsonSyntaxException("Unknown item '" + name + "'"));
        if (json.has("data")) {
    ...
    public static class Serializer implements RecipeSerializer<ShapedInfusionRecipe> {
        ...
        public ShapedInfusionRecipe read(Identifier identifier, JsonObject json) {
            ...
            ResourceAmount<FluidVariant> fluid = FluidUtils.fromJson(JsonHelper.getObject(json, "fluid"));
            ...
            return new ShapedInfusionRecipe(identifier, width, height, ingredients, fluid, result, duration);
        }
