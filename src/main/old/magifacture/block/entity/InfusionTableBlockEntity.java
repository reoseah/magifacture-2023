...
import reoseah.magifacture.recipe.InfusionRecipe;
import reoseah.magifacture.screen.InfusionTableScreenHandler;
import reoseah.magifacture.util.FluidUtils;

import java.util.Optional;

public class InfusionTableBlockEntity extends ProcessingBlockEntity<InfusionRecipe> implements SidedInventory {
    ...
    public static final BlockEntityType<InfusionTableBlockEntity> TYPE = FabricBlockEntityTypeBuilder.create(InfusionTableBlockEntity::new, InfusionTableBlock.INSTANCE).build();
    ...
    protected ScreenHandler createScreenHandler(int syncId, PlayerInventory playerInventory) {
        return new InfusionTableScreenHandler(syncId, this, playerInventory);
    }

    public static void tickServer(World world, BlockPos pos, BlockState state, InfusionTableBlockEntity be) {
    ...
    protected Optional<InfusionRecipe> findRecipeInternal(@NotNull World world) {
        return world.getRecipeManager().getFirstMatch(InfusionRecipe.TYPE, this, world);
    }

    @Override
    protected boolean canCraft(InfusionRecipe recipe) {
        return recipe.matches(this, this.world) && this.canFullyAddStack(SLOT_OUTPUT, recipe.craft(this));
    }

    @Override
    protected void craftRecipe(InfusionRecipe recipe) {
        this.addStack(SLOT_OUTPUT, recipe.craft(this));
